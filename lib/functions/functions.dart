...
validateEmail(email) async {
  ...
  try {
    ...
    if (e is SocketException) {
      ...
      result = e.toString();
      debugPrint('registerDriver error: $e');
    }
  }
  return result;
}
...
darktheme() async {
  if (isDarkTheme == true) {
    ...
    buttonColor = theme;
    loaderColor = theme;
    hintColor = const Color(0xff12121D).withValues(alpha: 0.3);
  }
  // assets/dark.json
  if (isDarkTheme == true) {
...
registerDriver() async {
  ...
  try {
    ...
    String fetchedPhone = phnumber;
    String fetchedCountryCode = countries[phcode]['dial_code'].toString().substring(1);
    int fetchedpncodeCode = phcode ?? 1;
    ...
    response.fields.addAll({
      ...
      "email": email,
      "device_token": fcm,
      "country": countries[fetchedpncodeCode]['code'],
      "country_code": fetchedCountryCode,
      // "country_code": countries[phcode]['dial_code'].isEmpty
      //     ? fetchedCountryCode
      //     : countries[phcode]['dial_code'].toString().substring(1),
...
registerOwner() async {
  ...
  try {
    ...
    response.headers.addAll({'Content-Type': 'application/json'});
    if (proImageFile != null) {
      debugPrint('profile_picture not null: ${proImageFile.toString()}');

      response.files.add(
          await http.MultipartFile.fromPath('profile_picture', proImageFile));
    }

    String fetchedPhone = sharefPref!.getString(phoneForPageRedirect)!;
    String fetchedCountryCode = sharefPref!.getString(countryForPageRedirect)!;
    int fetchedpncodeCode = sharefPref!.getInt(phcodeForRedirect) ?? 1;
    response.fields.addAll({
      "name": name,
      // "mobile": phnumber,
      "mobile": phnumber.isEmpty ? fetchedPhone : phnumber,
      "email": email,
      ...
      "country": countries[fetchedpncodeCode]['code'],
      // "country_code": countries[phcode]['dial_code'].toString().substring(1),
      "country_code": fetchedCountryCode,
      "service_location_id": myServiceId.toString(),
      "login_by": (platform == TargetPlatform.android) ? 'android' : 'ios',
      'lang': choosenLanguage,
    });
    debugPrint(response.fields.toString());
    var request = await response.send();
    var respon = await http.Response.fromStream(request);
  ...
  try {
    var response = await http.post(
        ...
        body: (value == 0)
            ? {
          "mobile":
          '${countries[phcode]['dial_code'].toString().substring(1)}$number',
    ...
    if (userDetails.isNotEmpty && userDetails['role'] == 'driver') {
      ...
      if (userDetails['active'] == true &&
        ...
        await geolocs.Geolocator.getCurrentPosition(
            desiredAccuracy: geolocs.LocationAccuracy.best);
      }
...
// google map to pickup location filled
launchGoogleMapToPickup(lat, lng) async {
...
// google map from pickup location to drop off location filled
launchGoogleMapFromPickupToDropoff(
    pickupLat, pickupLng, dropoffLat, dropoffLng) async {
  String routeUrl =
      'https://www.google.com/maps/dir/?api=1&origin=$pickupLat,$pickupLng&destination=$dropoffLat,$dropoffLng&travelmode=driving';
  ...
  try {
    ...
    if (await canLaunch(routeUrl)) {
      ...
      throw 'Could not launch $routeUrl';
    }
  } catch (e) {
    if (e is SocketException) {
      internet = false;
    } else {
      debugPrint('Error launching map route: $e');
    }
  }
}

// Waze map to pickup location filled

launchWazeMapToPickup(lat, lng) async {
  String destinationUrl = 'https://waze.com/ul?ll=$lat,$lng&navigate=yes';
  ...
  try {
    ...
    if (e is SocketException) {
      ...
      debugPrint('Error launching Waze map: $e');
    }
  }
}

// waze map from pickup location to drop off location filled

launchWazeMapFromPickupToDropoff(
    pickupLat, pickupLng, dropoffLat, dropoffLng) async {
  String routeUrl =
      'https://waze.com/ul?ll=$pickupLat,$pickupLng&navigate=yes&to=$dropoffLat,$dropoffLng';
  ...
  try {
    ...
    if (e is SocketException) {
      ...
      debugPrint('Error launching Waze map route: $e');
    }
  }
}
...
sendMessage(chat) async {
  ...
  try {
    ...
    debugPrint('sendChat response: ${response.statusCode}');
    if (response.statusCode == 200) {
      getCurrentMessages();
      FirebaseDatabase.instance
          .ref('requests/${driverReq['id']}')
...
userLogout() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      debugPrint('Response of Logout: ${response.statusCode}');
...
geolocs.LocationSettings locationSettings = (platform == TargetPlatform.android)
    ...
    accuracy: geolocs.LocationAccuracy.best,
    foregroundNotificationConfig:
    const geolocs.ForegroundNotificationConfig(
      notificationText:
      "Rungo Driver will continue to receive your location in background",
...
Future<LatLng?> getLatLngFromAddress(String address) async {
  try {
    // Convert address to place ID
    final placeSearchResponse = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=$address&inputtype=textquery&fields=place_id&key=$mapkey'));
    if (placeSearchResponse.statusCode == 200) {
      final placeSearchResult = jsonDecode(placeSearchResponse.body);
      if (placeSearchResult['candidates'].isEmpty) {
        debugPrint('No place found for the given address.');
        return null;
      }
      final placeId = placeSearchResult['candidates'][0]['place_id'];
      ...
      if (response.statusCode == 200) {
        var val = jsonDecode(response.body)['result']['geometry']['location'];
        return LatLng(val['lat'], val['lng']);
      } else {
        return null;
      }
    } else {
      debugPrint('Failed to fetch place ID: ${placeSearchResponse.body}');
      return null;
    }
  } catch (e) {
    debugPrint('Error occurred while fetching LatLng: $e');
    return null;
  }
}

// Drawing polylines by AK

drawPolylineBetweenPoints(LatLng source, LatLng destination) async {
  Set<Polyline> sourceTodestintionPolylines = {};
  sourceToDestPolylines = {};
  try {
    // Fetching the polyline points from Google Directions API
    final response = await http.get(Uri.parse(
        'https://maps.googleapis.com/maps/api/directions/json?origin=${source.latitude},${source.longitude}&destination=${destination.latitude},${destination.longitude}&key=$mapkey'));
    if (response.statusCode == 200) {
      final result = jsonDecode(response.body);
      if (result['routes'].isNotEmpty) {
        // Decode polyline points
        final points = result['routes'][0]['overview_polyline']['points'];
        final List<PointLatLng> decodedPoints = decodeEncodedPolyline(points);

        // Creating a polyline with the decoded points
        final Polyline polyline = Polyline(
          polylineId:
          PolylineId('poly_${source.hashCode}_${destination.hashCode}'),
          color: buttonColor,
          endCap: Cap.roundCap,
          startCap: Cap.roundCap,
        ...
        // Adding the created polyline to the set
        sourceTodestintionPolylines.add(polyline);
      } else {
    ...
    debugPrint('Error occurred while fetching directions: $e');
  }
  sourceToDestPolylines = sourceTodestintionPolylines;
}
...
//
void adjustZoomAndAnimateCamera(
    LatLng source, LatLng destination, dynamic controller) {
  LatLngBounds bound;
...
navigate(BuildContext context) {
  Navigator.pushAndRemoveUntil(context,
      MaterialPageRoute(builder: (context) => const Maps()), (route) => false);
}
...
class ButtonBottomSpace extends StatelessWidget {
  const ButtonBottomSpace({Key? key, this.height}) : super(key: key);
  final double? height;
  @override
  Widget build(BuildContext context) {
    return SizedBox(
      height: Responsive.height(height ?? 2, context),
    );
  }
}
...
class IosBackButton extends StatelessWidget {
  const IosBackButton({Key? key, this.width, this.height}) : super(key: key);
  final double? width;
  final double? height;
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: EdgeInsets.only(left: Responsive.width(1.8, context)),
      alignment: Alignment.center,
      width: width ?? Responsive.width(9, context),
      height: height ?? Responsive.width(9, context),
      decoration:
      BoxDecoration(shape: BoxShape.circle, color: darkModeSecContainer),
      child: Icon(Icons.arrow_back_ios,
          size: Responsive.width(5, context), color: textColor),
    );
  }
}
...
class InvoicePriceListItem extends StatelessWidget {
  const InvoicePriceListItem({required this.child});
  final Widget child;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          color: darkModeSecContainer, borderRadius: BorderRadius.circular(7)),
      child: child,
    );
  }
}

class MenuSubCatagoryItem extends StatelessWidget {
  const MenuSubCatagoryItem({required this.child});
  final Widget child;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
          color: darkModeSecContainer, borderRadius: BorderRadius.circular(15)),
      child: child,
    );
  }
}
...
// Updating driver token in firebase

updateDriverFcm() async {
  try {
    var token = await FirebaseMessaging.instance.getToken();
    if (token != null) {
      await FirebaseDatabase.instance
      ...
      debugPrint('No FCM token retrieved.');
    }
  } catch (e) {
...
checkFcmTokenChange() {
  FirebaseDatabase.instance
      ...
      .onValue
      .listen((event) {
    String currentToken = (event.snapshot.value as Map?)?['fcm_token'];
